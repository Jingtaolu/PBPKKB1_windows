class PbpkmodelsController < ApplicationController
  ...
  def show_models
    ...
    ranked_chemical_a_a=@tanimoto_rank.map {|a| a=a[0]["csid"]}
    ...
    pbpkmodels_a_h_calc_rank=pbpkmodels_a_h.each {|a| a[:similarity_ranking]=ranked_chemical.index(a["model_chemical_CSID"])}
    ...
    @pbpkmodels_chemical=pbpkmodels_a_h_calc_rank.map do |a|
      a.select {|k,v| k=="model_chemical_CSID" }["model_chemical_CSID"]
    end
  end
  ...
  def search_models
    ...
    pbpkmodels_active_record = Pbpkmodel.all
    pbpkmodels_a_h=pbpkmodels_active_record.as_json
    pbpkmodels_a_h.map {|a| a.delete("chemical")}

    @pbpkmodels_searched=pbpkmodels_a_h.select do |b|
      c=b.map {|k,v| v.include? params["q"].downcase  }
      c.include? true
    end
